...
export class Table<T> implements ITable<T> {
  ...
  public itemIds(): string[] {
    if (this._view) {
      ...
      const filteredIds = _filter
        ? allIds.filter((id) => _filter(this._items[id]!, this.getTablePathTokens()))
        : allIds;
      const orderedFilteredIds = _comparator
        ? filteredIds.sort((id1, id2) =>
            _comparator(this._items[id1]!, this._items[id2]!, this.getTablePathTokens())
          )
        : filteredIds;
  ...
  private setInternal(id: string, value: T | null, shouldFlag: boolean = true): boolean {
    ...
    if (
      ...
      (currentValue !== null && value !== null && this.isEqual(currentValue, value))
    ) {
      return false;
    }
  ...
  private applyViewUpdate(currentView: string[], updatedIds: string[]) {
    ...
    const updatedView = allocateEmptyArray<string>(
